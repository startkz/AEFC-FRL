...
assert(dpl.line_count==34 && dpl.parameter_equivalence_passed && dpl.topology_connectivity_passed, ...
    'DPL parameter/topology contract must pass before dynamic-state audit.');
...
report.minimum_positive_sequence_delay_steps=min(arrayfun(@(x)x.positive_sequence.delay_steps,dpl.lines));
report.maximum_positive_sequence_delay_steps=max(arrayfun(@(x)x.positive_sequence.delay_steps,dpl.lines));
...
% Record the native SPS DPL mask-level initialization fields.  These are not
% assumed to equal compiled electrical states; they are provenance only.
...
for i=1:numel(dpl.lines)
    ...
    lineInit{i}=struct('block',b,'fields',vals);
end
report.native_dpl_mask_initialization=lineInit;

% Compile-time initial block states from Simulink itself.
try
    oldFmt=get_param(mdl,'SaveFormat'); set_param(mdl,'SaveFormat','StructureWithTime');
    x0=Simulink.BlockDiagram.getInitialState(mdl);
    try, set_param(mdl,'SaveFormat',oldFmt); catch, end
...
% Specialized Power Systems electrical state-space diagnostic.  x0 is the
% state actually selected for simulation and xss is the SPS steady-state set.
try
    sps=power_analyze(mdl,'structure');
    pa=struct('available',true,'state_count',size(sps.A,1),'input_count',size(sps.B,2), ...
        'output_count',size(sps.C,1),'A_all_finite',all(isfinite(sps.A(:))), ...
        'x0_all_finite',all(isfinite(sps.x0(:))),'x0_max_abs',maxabs(sps.x0), ...
        'rcond_A',safe_rcond(sps.A),'frequencies',double(sps.frequencies(:).'));
    if isfield(sps,'xss') && ~isempty(sps.xss)
        pa.xss_all_finite=all(isfinite(sps.xss(:))); pa.xss_max_abs=maxabs(sps.xss);
        if ~isempty(sps.frequencies)
...
if isempty(trajectory.first_violation_s)
    report.first_violation_delay_steps=[];
    report.violation_before_minimum_dpl_propagation=false;
else
    report.first_violation_delay_steps=trajectory.first_violation_s/report.native_step_s;
    report.violation_before_minimum_dpl_propagation= ...
        report.first_violation_delay_steps < report.minimum_positive_sequence_delay_steps;
end
...
fprintf('IEEE39 DPL dynamic-state audit: minDelay=%.6g Ts firstViolation=',report.minimum_positive_sequence_delay_steps);
if isempty(report.first_violation_delay_steps), fprintf('none'); else, fprintf('%.6g Ts',report.first_violation_delay_steps); end
fprintf(' beforeMinDelay=%d\n',logical(report.violation_before_minimum_dpl_propagation));
if isfield(report,'power_analyze') && isfield(report.power_analyze,'available') && report.power_analyze.available
    fprintf('  SPS states=%d rcond(A)=%.6g x0max=%.6g\n',report.power_analyze.state_count,report.power_analyze.rcond_A,report.power_analyze.x0_max_abs);
end
end
...
if isstruct(x0) && isfield(x0,'signals')
    ...
    for i=1:numel(sig)
        ...
        records{i}=struct('block',sig(i).blockName,'label',sig(i).label,'sample_time',sig(i).sampleTime, ...
            'finite',fin,'max_abs',m,'dimensions',sig(i).dimensions);
    end
else
...
wm=cell(1,10); vv=cell(1,10); commonT=[];
for g=1:10
    [tw,dw]=timeseries_data(out.get(sprintf('aefc_dyn_wm_g%d',g))); [tv,dv]=timeseries_data(out.get(sprintf('aefc_dyn_vabc_g%d',g)));
    dv=reshape_vabc(dv); vpu=sqrt(mean(double(dv).^2,2))/(vbase(g)/sqrt(3));
    wm{g}=struct('t',tw,'v',double(dw(:))); vv{g}=struct('t',tv,'v',double(vpu(:)));
    if isempty(commonT), commonT=tw(:); end
end
checkSteps=[0 1 2 4 10 20 40 80 200]; points=cell(1,numel(checkSteps)); first=[];
for k=1:numel(checkSteps)
    target=checkSteps(k)*Ts; wvals=nan(1,10); vvals=nan(1,10);
    for g=1:10, wvals(g)=sample_at(wm{g}.t,wm{g}.v,target); vvals(g)=sample_at(vv{g}.t,vv{g}.v,target); end
    ...
    if isempty(first) && ~sane, first=target; end
end
% Search all logged samples for the earliest exact violation.
allTimes=unique(vertcat(wm{1}.t(:),vv{1}.t(:))); allTimes=allTimes(allTimes>=0 & allTimes<=stopTime);
for q=1:numel(allTimes)
    ...
    if ~sane, first=allTimes(q); break; end
end
tr.checkpoints=points; tr.first_violation_s=first; tr.logged_time_count=numel(allTimes);
end
...
function v=reshape_vabc(v), v=squeeze(double(v)); if isvector(v)&&numel(v)==3, v=reshape(v,1,3); end; if size(v,2)~=3&&size(v,1)==3, v=v.'; end; assert(size(v,2)==3); end
...
add_block('simulink/Sinks/To Workspace',lp,'VariableName',varName,'SaveFormat','Timeseries','Position',[180 y-2 300 y+16]); add_line(mdl,[fn '/1'],[ln '/1'],'autorouting','on');
